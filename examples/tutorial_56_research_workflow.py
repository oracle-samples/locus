...
"""Tutorial 56: Research workflow — execute → causal → summarize → grounding → replan.

``create_research_workflow`` composes six independent node primitives into
a StateGraph that mirrors the production pattern used in Optic's specialist
agents: a ReAct loop that gathers evidence, causal inference before summary,
LLM-as-judge grounding evaluation, and a two-level recovery loop.
...
1. Running ``create_research_workflow`` with the convenience factory.
2. Subscribing to ``research.*`` SSE events in real time.
3. Composing custom graphs from individual node primitives.
4. Reading the final state: summary, structured output, grounding score.

Difficulty: intermediate. Prerequisites: tutorial_52 (observability basics),
tutorial_41 (DeepAgent factory).

Run with:
    python examples/tutorial_56_research_workflow.py
"""
...
# =============================================================================
# Domain: tiny module catalogue the agent can investigate
# =============================================================================
...
def describe_module(name: str) -> dict:
    ...
    Args:
        name: Dotted module name (e.g. ``locus.router``).
    """
...
# =============================================================================
# Part 1 — convenience factory + SSE stream
# =============================================================================


async def part1_factory_with_sse() -> None:
    """Run create_research_workflow and stream every research.* event."""
    ...
    workflow = create_research_workflow(
        ...
        system_prompt=(
            "You are a locus SDK analyst. Use tools to survey the available modules. "
    ...
    async with run_context() as rid:
        ...
        result = await workflow.execute({KEY_PROMPT: "Survey all locus modules."})
...
# =============================================================================
# Part 2 — compose custom graph from primitives
# =============================================================================


async def part2_custom_graph() -> None:
    """Build a minimal graph manually — execute + summarize only, no causal."""
    print("\n--- Part 2: custom graph (no causal inference) ---")

    from locus.deepagent.workflow import (  # noqa: PLC0415
        KEY_EVIDENCE,
        make_execute_node,
    ...
    async with run_context() as rid:
        result = await workflow.execute({KEY_PROMPT: "What does locus.router do?"})
        await get_event_bus().close_stream(rid)
