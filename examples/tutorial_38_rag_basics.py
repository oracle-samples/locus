...
Run it:
    ...
    python examples/tutorial_38_rag_basics.py

    # Offline (skips the live demo cleanly when env vars are missing):
    LOCUS_MODEL_PROVIDER=mock python examples/tutorial_38_rag_basics.py
...
def _get_store(table_suffix: str, dimension: int = 1024) -> OracleVectorStore:
    return OracleVectorStore(
        ...
        table_name=f"locus_tutorial_38_{table_suffix}",
        dimension=dimension,
...
async def using_vector_store():
    ...
    print(f"Created OracleVectorStore table=locus_tutorial_38_manual dim={store.config.dimension}")
