...
``VECTOR_DISTANCE`` SQL function. Other backends (pgvector,
OpenSearch) are valid alternatives; the Locus interface is identical.
...
Run it:
    ...
    python examples/notebook_44_rag_providers.py

    # Offline (skips the live demo cleanly when env vars are missing):
    LOCUS_MODEL_PROVIDER=mock python examples/notebook_44_rag_providers.py
...
def _store(
    ...
    **overrides,
    ...
    base = {
        ...
        "table_name": f"locus_notebook_39_{table_suffix}",
        "dimension": dimension,
