...
Run it (OCI Generative AI is the default; auto-detected from `~/.oci/config`):
    ...
    python examples/notebook_64_contract_review.py

Offline:

    LOCUS_MODEL_PROVIDER=mock python examples/notebook_64_contract_review.py

Pin a strong-enough model for the structured ContractDecision schema:

    LOCUS_MODEL_ID=openai.gpt-4.1 python examples/notebook_64_contract_review.py

## Source

```python
--8<-- "examples/notebook_64_contract_review.py"
```
