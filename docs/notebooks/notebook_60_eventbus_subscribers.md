...
Run it (OCI Generative AI is the default; auto-detected from `~/.oci/config`):
    ...
    python examples/notebook_60_eventbus_subscribers.py

Offline:

    LOCUS_MODEL_PROVIDER=mock python examples/notebook_60_eventbus_subscribers.py

## Source

```python
--8<-- "examples/notebook_60_eventbus_subscribers.py"
```
