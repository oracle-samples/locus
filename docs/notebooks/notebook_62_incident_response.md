...
Run it (OCI Generative AI is the default; auto-detected from `~/.oci/config`):
    ...
    python examples/notebook_62_incident_response.py

Offline:

    LOCUS_MODEL_PROVIDER=mock python examples/notebook_62_incident_response.py

Pin a strong-enough model for the structured postmortem schema:

    LOCUS_MODEL_ID=openai.gpt-4.1 python examples/notebook_62_incident_response.py

## Source

```python
--8<-- "examples/notebook_62_incident_response.py"
```
