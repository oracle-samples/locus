...
Run it (OCI Generative AI is the default; auto-detected from `~/.oci/config`):
    ...
    python examples/notebook_51_guardrails_security.py

Offline / no credentials:

    LOCUS_MODEL_PROVIDER=mock python examples/notebook_51_guardrails_security.py

Pin a specific OCI model:

    LOCUS_MODEL_ID=openai.gpt-4.1 python examples/notebook_51_guardrails_security.py

## Source

```python
--8<-- "examples/notebook_51_guardrails_security.py"
```
