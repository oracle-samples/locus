...
Run it (OCI Generative AI is the default; auto-detected from `~/.oci/config`):
    ...
    python examples/notebook_57_cognitive_router.py

Offline (uses fallback frames):

    LOCUS_MODEL_PROVIDER=mock python examples/notebook_57_cognitive_router.py

## Source

```python
--8<-- "examples/notebook_57_cognitive_router.py"
```
