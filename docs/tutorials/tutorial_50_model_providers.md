...
Run it (OCI Generative AI is the default; auto-detected from `~/.oci/config`):
    ...
    python examples/tutorial_50_model_providers.py

Offline:

    LOCUS_MODEL_PROVIDER=mock python examples/tutorial_50_model_providers.py

Pin a specific OCI model:

    LOCUS_MODEL_ID=meta.llama-3.3-70b-instruct python examples/tutorial_50_model_providers.py

## Source

```python
--8<-- "examples/tutorial_50_model_providers.py"
```
