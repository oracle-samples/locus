...
Run it (OCI Generative AI is the default; auto-detected from `~/.oci/config`):
    ...
    python examples/tutorial_52_cognitive_router.py

Offline (uses fallback frames):

    LOCUS_MODEL_PROVIDER=mock python examples/tutorial_52_cognitive_router.py

## Source

```python
--8<-- "examples/tutorial_52_cognitive_router.py"
```
