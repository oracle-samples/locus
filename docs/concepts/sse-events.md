...
Bridged from the agent's yielded `LocusEvent` stream by
...
1. Add a constant in `src/locus/observability/emit.py`:
...
2. Emit at the call site:
   ...
   ```python
   from locus.observability.emit import EV_FOO_BAR, emit
