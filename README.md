...
| **[💾 Durable memory](https://oracle-samples.github.io/locus/concepts/checkpointers/)** | Four native checkpointers (OCI Object Storage, in-memory, file, HTTP) plus five storage backends (PostgreSQL, OpenSearch, Redis, SQLite, Oracle Database 26ai). One `BaseCheckpointer` protocol — native checkpointers implement it directly; storage backends adapt through a thin `StorageBackendAdapter` so the agent runtime sees one shape regardless. |
