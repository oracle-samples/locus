...
class TestTutorial20CheckpointBackends:
    ...
    async def test_sqlite_backend(self):
        """Test SQLite backend with simple dict API."""
        ...
        try:
            ...
            # Save and load (simple dict API)
            await backend.save("thread_1", {"message": "Hello"})
            ...
            # Get metadata
            meta = await backend.get_metadata("thread_1")
...
class TestTutorialExecution:
    ...
    async def test_tutorial_13_runs(self):
        ...
        result = subprocess.run(
            [sys.executable, "examples/tutorial_13_structured_output.py"],
            capture_output=True,
            text=True,
            timeout=60,
            check=False,
    ...
    @pytest.mark.asyncio
    async def test_tutorial_14_runs(self):
        """Test that tutorial 14 runs without error."""
        import subprocess
        import sys

        result = subprocess.run(
            [sys.executable, "examples/tutorial_14_reasoning_patterns.py"],
            capture_output=True,
            text=True,
            timeout=60,
            check=False,
        )
        assert result.returncode == 0, f"Tutorial 14 failed: {result.stderr}"

    @pytest.mark.asyncio
    async def test_tutorial_15_runs(self):
        """Test that tutorial 15 runs without error."""
        import subprocess
        import sys

        result = subprocess.run(
            [sys.executable, "examples/tutorial_15_playbooks.py"],
            capture_output=True,
            text=True,
            timeout=60,
            check=False,
        )
        assert result.returncode == 0, f"Tutorial 15 failed: {result.stderr}"

    @pytest.mark.asyncio
    async def test_tutorial_20_runs(self):
        """Test that tutorial 20 runs without error."""
        import subprocess
        import sys

        result = subprocess.run(
            [sys.executable, "examples/tutorial_20_checkpoint_backends.py"],
            capture_output=True,
            text=True,
            timeout=60,
            check=False,
        )
        assert result.returncode == 0, f"Tutorial 20 failed: {result.stderr}"

    @pytest.mark.asyncio
    async def test_tutorial_21_runs(self):
        """Test that tutorial 21 runs without error."""
        import subprocess
        import sys

        result = subprocess.run(
            [sys.executable, "examples/tutorial_21_sse_streaming.py"],
            capture_output=True,
            text=True,
            timeout=60,
            check=False,
        )
        assert result.returncode == 0, f"Tutorial 21 failed: {result.stderr}"
