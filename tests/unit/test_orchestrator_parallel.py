...
deliberate trade for OCI GenAI stability before timeouts were
properly pinned). With ``OCIOpenAIModel`` now sharing the OpenAI
provider's ``request_timeout`` / ``max_retries`` defaults, the
...
from locus.multiagent.orchestrator import Orchestrator, RoutingDecision
...
async def test_empty_first_response_is_retried() -> None:
    ...
    # implementation — it covers OCI GenAI's occasional empty-completion
