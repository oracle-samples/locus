...
"""Tests for secret redaction patterns in ``locus.tools.executor``.
...
class TestExistingPatterns:
    ...
    def test_ocid_redacted(self) -> None:
        out = redact_sensitive_text("tenancy ocid1.tenancy.oc1..aaaaaaaaabcdef is bad")
        assert "ocid1.tenancy" not in out
...
class TestVendorPrefixPatterns:
    @pytest.mark.parametrize(
        "token",
        [
            "sk-ant-api03-srfILj2QMXXsgp4Re4QxblnEeIJm_zCN6g0G0wSzsgy",
            "sk-proj-KXOiNAJ9PGGIeOSZYf4OnM7WASY8kV8Y2eySwx5AzOhClfWzL1ime0Yj5w",
            "sk-abcdefghijklmnopqrstuvwxyz0123456789ABCD",  # ≥32 chars
